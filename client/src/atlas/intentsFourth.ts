 ...
 */
import { est1rm, muscleSetsInWorkout, resolveMuscles, setTopWeight, setTypeOf, topSet } from '../store';
...
export const INTENTS_FOURTH: Intent[] = [
  ...
  {
    id: 'plate_math',
    all: [['plates', 'plate', 'load the bar', 'load', 'loading', 'млинц*', 'блин*', 'навісити', 'повісити']],
    answer: (c, p, L) => {
      const kg = numbers(p.phrase).find((n) => n >= 20 && n <= 500);
      if (!kg) return L('Tell me the total, e.g. “plates for 100 kg”.', 'Скажи загальну вагу, напр. «млинці на 100 кг».');
  ...
  {
    id: 'reps_at_weight',
    all: [['how many reps', 'reps at', 'reps with', 'скільки повторів', 'скільки разів', 'на скільки повторів']],
    needs: 'exercise',
    answer: (c, p, L) => {
      ...
      return reps < 1
        ? L(`${name} at ${c.fmt.kg(kg)}: above your estimated max (~${c.fmt.kg(Math.round(e1))}). Not today.`, `${name} з ${c.fmt.kg(kg)}: понад твій розрахунковий максимум (~${c.fmt.kg(Math.round(e1))}). Не сьогодні.`)
        : L(`${name} at ${c.fmt.kg(kg)}: about ${Math.min(reps, 30)} reps to failure, going by your recent sets.`, `${name} з ${c.fmt.kg(kg)}: приблизно ${Math.min(reps, 30)} повторів до відмови, судячи з останніх сетів.`);
    },
  },
  {
    id: 'percent_max',
  ...
  {
    id: 'exercise_muscles',
    all: [['what muscles', 'which muscles', 'what does', 'works', 'target*', 'які м’язи', 'які мязи', 'що працює', 'на що', 'що качає']],
    needs: 'exercise',
    answer: (c, p, L) => {
  ...
  {
    id: 'alternatives',
    all: [['alternative*', 'instead of', 'replace', 'swap', 'substitut*', 'альтернатив*', 'замість', 'замінит*', 'чим замінити']],
    needs: 'exercise',
    answer: (c, p, L) => {
      const r = resolveMuscles({ name: p.exercise!, kind: 'strength' } as Exercise);
      if (!r.primary) return null;
      ...
      for (const w of finishedOf(c)) if (w.gymId) counts.set(w.gymId, (counts.get(w.gymId) ?? 0) + 1);
  ...
  },
  {
    ...
    answer: (c, p, L) => {
      const counts = finishedOf(c)
        .slice(0, 10)
        .map((w) => w.exercises.find((e) => e.name === p.exercise)?.sets.filter((s) => setTypeOf(s) !== 'warmup').length ?? 0)
        .filter((n) => n > 0);
      const usual = counts.length ? Math.round(counts.reduce((a, b) => a + b, 0) / counts.length) : null;
      return L(
  ...
  {
    id: 'day_lookup',
    all: [['what did i do', 'what did i train', 'on monday', 'on tuesday', 'on wednesday', 'on thursday', 'on friday', 'on saturday', 'on sunday', 'що я робив', 'що я тренував', 'в понеділок', 'у понеділок', 'у вівторок', 'в середу', 'у середу', 'в четвер', 'у четвер', 'в пятницю', 'у пятницю', 'в суботу', 'у суботу', 'в неділю', 'у неділю']],
    answer: (c, p, L) => {
      const day = WEEKDAYS.find(([, kws]) => kws.some((k) => p.words.some((w) => w.startsWith(k.replace('*', '')))));
      if (!day) return null;
      const w = finishedOf(c).find((x) => new Date(x.startedAt).getDay() === day[0] && c.now - x.startedAt < WEEK + DAY);
      if (!w) return L(`Nothing logged last ${wd(c, day[0])}.`, `Минулого разу в ${wd(c, day[0])} нічого не записано.`);
      const lifts = w.exercises
        .filter((e) => e.sets.some((s) => setTypeOf(s) !== 'warmup'))
        .map((e) => {
          const t = topSet(e.sets);
          return t ? `${c.fmt.exercise(e.name)} ${c.fmt.kg(setTopWeight(t))}×${t.reps}` : c.fmt.exercise(e.name);
        })
        .slice(0, 5)
        .join(', ');
      return L(`${wd(c, day[0])} (${date(c, w.startedAt)}): ${lifts}.`, `${wd(c, day[0])} (${date(c, w.startedAt)}): ${lifts}.`);
    },
  },
  {
    id: 'compare_weeks',
    all: [['last week', 'previous week', 'week before', 'минулого тижня', 'минулим тижнем', 'попереднім тижнем'], ['compare', 'vs', 'versus', 'than', 'порівня*', 'ніж', 'проти']],
    answer: (c, _p, L) => {
      const f = finishedOf(c);
      const count = (from: number, to: number) => f.filter((w) => w.startedAt >= from && w.startedAt < to);
      const a = count(c.now - WEEK, c.now + 1);
      const b = count(c.now - 2 * WEEK, c.now - WEEK);
      const sets = (ws: typeof f) => ws.reduce((n, w) => n + [...muscleSetsInWorkout(w).values()].reduce((x, y) => x + y, 0), 0);
      return L(
  ...
  {
    id: 'short_on_time',
    all: [['only', 'no time', 'short on time', 'quick', 'minutes', 'тільки', 'нема часу', 'немає часу', 'мало часу', 'швидко', 'хвилин*']],
    answer: (_c, p, L) => {
      const m = numbers(p.phrase).find((n) => n >= 10 && n <= 90);
      if (!m && !p.phrase.match(/no time|нема часу|немає часу|мало часу|short on time|quick|швидко/)) return null;
      return L(
  ...
  {
    id: 'two_days_row',
    all: [['two days in a row', 'back to back', 'every day', 'consecutive', 'два дні поспіль', 'два дні підряд', 'кожен день', 'щодня']],
    answer: (_c, _p, L) =>
      L(
  ...
  {
    id: 'two_a_day',
    all: [['twice a day', 'two a day', 'two sessions a day', 'двічі на день', 'два рази на день', 'два тренування на день']],
    answer: (_c, _p, L) =>
      L(
  ...
  // ---- health & safety edges ----
  {
    ...
    all: [['blood pressure', 'hypertension', 'diabetes', 'asthma', 'heart condition', 'heart disease', 'arrhythm*', 'тиск*', 'гіпертон*', 'діабет*', 'астм*', 'серц*', 'аритм*']],
    neutral: true,
    answer: (_c, _p, L) =>
      L(
  ...
  {
    id: 'peds',
    all: [['steroid*', 'sarm*', 'testosterone', 'trt', 'peds', 'anabolic*', 'стероїд*', 'анабол*', 'тестостерон*', 'хімі*', 'фарма']],
    neutral: true,
    answer: (_c, _p, L) =>
      L(
  ...
  {
    id: 'running',
    all: [['run', 'running', 'jog*', '5k', '10k', 'marathon', 'біг*', 'бігат*', 'пробіжк*', 'марафон*']],
    answer: (_c, _p, L) =>
      L(
  ...
  {
    id: 'other_sport',
    all: [['football', 'soccer', 'boxing', 'bjj', 'mma', 'tennis', 'basketball', 'climbing', 'cycling', 'swimming', 'футбол*', 'бокс*', 'єдиноборств*', 'теніс*', 'баскетбол*', 'скелелаз*', 'плаван*', 'вело'], ['lift*', 'gym', 'strength', 'weights', 'train*', 'силов*', 'зал*', 'тренув*', 'кача*']],
    answer: (_c, _p, L) =>
      L(
  ...
  {
    id: 'home_equipment',
    all: [['home gym', 'what to buy', 'buy equipment', 'dumbbells for home', 'домашн* зал', 'що купити', 'купити гантел*', 'обладнання для дому']],
    answer: (_c, _p, L) =>
      L(
  ...
  {
    id: 'what_can_i_ask',
    all: [['what can i ask', 'examples', 'what to ask', 'що можна спитати', 'що питати', 'приклади питань']],
    answer: (_c, _p, L) =>
      L(
    ...
    suggest: (L) => [L('What should I train today?', 'Що тренувати сьогодні?'), L('How am I doing?', 'Як мій прогрес?'), L('Plates for 100 kg', 'Млинці на 100 кг')],
  },
  {
    id: 'reminder',
